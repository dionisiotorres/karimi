...
class StandardMedium(models.Model):
	...
	def _onchange_time(self):
		time_from = ''
		time_to = ''
		print ("==================self.hour_from",self.hour_from)
		if self.hour_from:
...
class StandardStandard(models.Model):
	...
	is_shift = fields.Selection([
		('yes', 'Yes'),
		('no', 'No'),
	], string='You Want Shift?', default='yes')
	...
	def _get_s_no(self):
		self.s_no = len(self.semester_ids)+1


class TeacherClassTimetable(models.Model):
	_name = 'teacher.class.timetable'

	
	
	@api.model
	def default_get(self,vals):
		print ("============self._xontext",self._context)
		result = super(TeacherClassTimetable, self).default_get(vals)
# 		res = self.search([('class_id','=', self._context.get('active_id') or False)],limit=1,order='id desc')
		oyd = False
		tyd = False
# 		print ("============self.env['school.standard'].browse(self._context.get('active_id')).standard_id.name == 'One Year Diploma'",self.env['school.standard'].browse(self._context.get('active_id')).standard_id.name == 'One Year Diploma')
		if self.env['school.standard'].browse(self._context.get('active_id')).standard_id.name == 'One Year Diploma':
			oyd = True
		else:
			tyd = True
		result.update({'oyd':oyd,'tyd':tyd})
# 		if res:
# 			result.update({
# 				'class_id': res.class_id.id,
# 				'oyd_first_teacher':res.oyd_first_teacher.id,
# 				'oyd_first_teacher_start_time':res.oyd_first_teacher_start_time,
# 				'oyd_first_teacher_end_time':res.oyd_first_teacher_end_time,
# 				'oyd_first_subject':res.oyd_first_subject.id,
# 				
# 				'oyd_second_teacher':res.oyd_second_teacher.id,
# 				'oyd_second_teacher_start_time':res.oyd_second_teacher_start_time,
# 				'oyd_second_teacher_end_time':res.oyd_second_teacher_end_time,
# 				'oyd_second_subject':res.oyd_second_subject.id,
# 				
# 				
# 				'oyd':oyd,
# 				'tyd':tyd,
# 				'first_period_subject':res.first_period_subject.id,
# 				'first_period_start_time':res.first_period_start_time,
# 				'first_period_end_time':res.first_period_end_time,
# 				'second_period_subject':res.second_period_subject.id,
# 				'second_period_start_time':res.second_period_start_time,
# 				'second_period_end_time':res.second_period_end_time,
# 				'third_period_subject':res.third_period_subject.id,
# 				'third_period_start_time':res.third_period_start_time,
# 				'third_period_end_time':res.third_period_end_time,
# 				'fourth_period_subject':res.fourth_period_subject.id,
# 				'fourth_period_start_time':res.fourth_period_start_time,
# 				'fourth_period_end_time':res.fourth_period_end_time,
# 				'fifth_period_subject':res.fifth_period_subject.id,
# 				'fifth_period_start_time':res.fifth_period_start_time,
# 				'fifth_period_end_time':res.fifth_period_end_time,
# 				'six_period_subject':res.six_period_subject.id,
# 				'six_period_start_time':res.six_period_start_time,
# 				'six_period_end_time':res.six_period_end_time,
# 				'seven_period_subject':res.seven_period_subject.id,
# 				'seven_period_start_time':res.seven_period_start_time,
# 				'seven_period_end_time':res.seven_period_end_time,
# 				
# 				})
		return result
	name = fields.Char('Name')
	oyd_first_teacher = fields.Many2one('school.teacher', 'First Teacher')
	oyd_first_subject = fields.Many2one('subject.subject', 'First Subject')
	oyd_first_teacher_start_time = fields.Char("First Period Start Time")
	oyd_first_teacher_end_time = fields.Char("First Period Start Time")
	
	oyd_second_teacher = fields.Many2one('school.teacher', 'Second Teacher')
	oyd_second_subject = fields.Many2one('subject.subject', 'Second Subject')
	oyd_second_teacher_start_time = fields.Char("Second Period Start Time")
	oyd_second_teacher_end_time = fields.Char("Second Period Start Time")
	
	first_period_teacher = fields.Many2one('school.teacher', 'First Period Teacher')
	second_period_teacher = fields.Many2one('school.teacher', 'Second Period Teacher')
	third_period_teacher = fields.Many2one('school.teacher', 'Third Period Teacher')
	fourth_period_teacher = fields.Many2one('school.teacher', 'Fourth Period Teacher')
	fifth_period_teacher = fields.Many2one('school.teacher', 'Fifth Period Teacher')
	six_period_teacher = fields.Many2one('school.teacher', 'Six Period Teacher')
	seven_period_teacher = fields.Many2one('school.teacher', 'Seven Period Teacher')
	
	class_id = fields.Many2one('school.standard',string="Class")
	teacher_id = fields.Many2one('school.teacher', 'Teacher')
	start_time = fields.Char("Time From")
	end_time = fields.Char("Time To")
	subject = fields.Many2one('subject.subject','Subject')
	oyd = fields.Boolean(string= 'oyd')
	tyd = fields.Boolean(string='tyd')
	
	first_period_subject = fields.Many2one('subject.subject','First Period Subject')
	first_period_start_time = fields.Char("First Period Start Time")
	first_period_end_time = fields.Char("First Period End Time")
	
	second_period_subject = fields.Many2one('subject.subject','Second Period Subject')
	second_period_start_time = fields.Char("Second Period Start Time")
	second_period_end_time = fields.Char("Second Period End Time")
	
	third_period_subject = fields.Many2one('subject.subject','Third Period Subject')
	third_period_start_time = fields.Char("Third Period Start Time")
	third_period_end_time = fields.Char("Third Period End Time")
	
	fourth_period_subject = fields.Many2one('subject.subject','Fourth Period Subject')
	fourth_period_start_time = fields.Char("Fourth Period Start Time")
	fourth_period_end_time = fields.Char("Fourth Period End Time")
	
	fifth_period_subject = fields.Many2one('subject.subject','Fifth Period Subject')
	fifth_period_start_time = fields.Char("Fifth Period Start Time")
	fifth_period_end_time = fields.Char("Fifth Period End Time")
	
	six_period_subject = fields.Many2one('subject.subject','Sixth Period Subject')
	six_period_start_time = fields.Char("Sixth Period Start Time")
	six_period_end_time = fields.Char("Sixth Period End Time")
	
	seven_period_subject = fields.Many2one('subject.subject','Seven Period Subject')
	seven_period_start_time = fields.Char("Seven Period Start Time")
	seven_period_end_time = fields.Char("Seven Period End Time")
...
class SchoolStandard(models.Model):
		...
		if vals.get('sequence', _('New')) == _('New'):
			...
			if not seq:
				...
				seq.write({'alphabet' : order,
						   'number' : number})
		standard = self.search([('school_id','=',vals.get('school_id')),('semester_id','=',vals.get('semester_id')),('medium_id','=',vals.get('medium_id'))])
		if standard:
			temp_seq = []
			for a in standard:
				temp_seq.append(a.standard.split('/')[-1])
			cust_seq = chr(ord(temp_seq[-1]) + 1) 
		else:
			cust_seq = 'A'
		vals.update({
					'standard': school_code.code +'/'+ semester_code.code +'/'+ medium_code.code +'/'+ cust_seq
						 })
	...
	user_id = fields.Many2one('school.teacher', 'Class Teacher',readonly=True)
	...
	state = fields.Selection([
		('draft', 'New'),
		('running', 'Running'),
		('finish','Going-To-Finish'),
		('close', 'Closed'),
		
	], string='Status', default='draft')
	tv_user_id = fields.Many2one('res.users', string='TV User')
	qr_code = fields.Binary('QR Code')
	class_time_table = fields.One2many('teacher.class.timetable','class_id', string="Class Time Table")
	teacher_id = fields.Many2one('school.teacher',string="Teacher")


#('division_id', '=',self.division_id.id),Developer 2
	@api.constrains('standard_id','medium_id','state')
	def check_standard_unique(self):
		standard_search = self.env['school.standard'
											 ...
											 ('semester_id', '=',
											  self.semester_id.id),
											 ('division_id','=',self.class_room_id.id),
											 ('school_id', '=',
											  self.school_id.id),
											 ('state','!=','close'),
											 ('id', 'not in', self.ids)])
	...
	def send_class_status_new(self):
		...
		if self.state == "running":
			...
			for res in self.search([('end_date', '<', date_end),('state','=','running')]):
				...
				print "testingggggg"
				rec = self.env['student.regular.timetable'].search([('classes','=',self.standard)])
				for obj in rec:
					obj.status = self.state
	...
	def onchange_class_room(self):
		...
		for rec in data:
			if self.division_id.name==rec.division_id.name :
				...
				if self.school_id.name==rec.school_id.name :
					...
					# 		delta=d2-d3
					# 		print "gjfghjfdghdjghdfhngjhdfhg"
...
class GradeMaster(models.Model):
	...
	name = fields.Many2one('standard.standard',required=True)
...
class StudentTransfer(models.Model):
	...
	@api.constrains('school_id','medium_id')
	def class_transfer_validation(self):
		if self.from_campus == self.school_id and self.shift_id == self.medium_id:
			raise ValidationError('Please select Valid Transfer Campus name and Shift name')
	...
	@api.model
	def default_get(self,vals):
		res = super(StudentTransfer, self).default_get(vals)
		if self._context.get('active_id'):
			res['student_name']=self._context.get('active_id')
		return res
	...
	def _compute_student_school(self):
		if self.student_name:
			...
			# 	'/' + self.student_name.standard_id.division_id.code))F
	...
	def set_close(self):
		...
		if var:
			ele = {
				...
				'from_campus_id': self.from_campus.name,
...
class StudentWithdrawal(models.Model):
	...
	def default_get(self,value):
		res = super(StudentWithdrawal, self).default_get(value)
		if self._context.get('active_id'):
			res['student_name']=self._context.get('active_id')
		return res
...
class TeacherTransfer(models.Model):
	...
	date = fields.Date('Requested Date',default=datetime.now())
	...
	to_location=fields.Char('To Location',compute='_compute_campus_address', store=True)
	...
	def _compute_campus_address(self):
		if self.school_id:
			...
			self.to_location=self.school_id.location
	...
	def set_close(self):
		...
		if rec.school_id:
			...
			rec.address_id.street=self.to_address
			rec.work_location = self.to_location
		
		teacher_var = self.env['school.teacher'].search([('id','=',self.teacher_id.id)])
		if teacher_var:
			teacher_var.school_id=self.school_id
			teacher_var.address_id.street=self.to_address
			teacher_var.work_location=self.to_location
		# rec=self.env['hr.employee'].search([('name','=',self.teacher_id.name)])
		# if rec.school_id:
		# 	rec.school_id=self.school_id
