...
class SchoolTeacher(models.Model):
	...
	def action_view_class(self):
		class_count = self.env['school.standard'].search([('teacher_id','=',self.id)])
		print ("==============class_id",class_count)
		action = self.env.ref('school.action_school_standard_form').read()[0]
		if len(class_count) > 1:
			action['domain'] = [('id', 'in', class_count.ids)]
		elif len(class_count) == 1:
			action['views'] = [(self.env.ref('school.view_school_standard_form').id, 'form')]
			action['res_id'] = class_count.ids[0]
		else:
			return
		return action
	...
	def _compute_class_count(self):
		class_count = self.env['school.standard'].search([('teacher_id','=',self.id)])
		self.class_count= len(class_count)

	class_count = fields.Integer('Class', compute='_compute_class_count')

	employee_id = fields.Many2one('hr.employee', 'Employee ID',
								  ondelete="cascade",
								  delegate=True, required=True)
	standard_id = fields.Many2many('school.standard','standard_teacher_rel',
								  'std_id', 'sub_id',
								  "Responsibility of Academic Class",
								  help="Standard for which the teacher\
								  responsible for.")
	stand_id = fields.Many2one('standard.standard', "Program",
	...
	examiner = fields.Boolean('Is Examiner')
	...
	employee_code=fields.Char('Code')

	teacher_transfer_ids = fields.One2many('teacher.transfer.line','teacher_transfer_id')
...
class SchoolTeacherTransfer(models.Model):
	''' Teacher Transfer '''
	_name = 'school.teacher.transfer'
	...
	school_id = fields.Many2one('school.school', "Campus",compute='_depends_teacher',store=True)
...
#developer2
	...
	def unlink(self):
		...
		return super(SchoolTeacherTransfer, self).unlink()

	@api.multi
	def set_to_transfer(self):
		if self.user_id:
			self.user_id.is_examiner = True
			self.user_id.examiner = True
			self.user_id.is_teacher = False
	...
	def set_reject(self):
		if self.user_id:
			self.user_id.is_examiner = False
			self.user_id.examiner = False
			self.user_id.is_teacher = True
		self.write({'state': 'reject'})

class TeacherTransferLine(models.Model):
	_name = 'teacher.transfer.line'
	_description = 'Teacher Transfer Line'

	teacher_transfer_id = fields.Many2one('school.teacher')

	teacher_name_id = fields.Many2one('school.teacher', 'Teacher')
	teacher_from_campus = fields.Char("Transferred From")
	teacher_school_id = fields.Many2one('school.school', "To Campus")
	teacher_reason = fields.Text("Reason")
	teacher_date = fields.Datetime('Date')
