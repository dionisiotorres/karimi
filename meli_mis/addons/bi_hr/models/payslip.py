...
class Employee(models.Model):
	...
	#Developer2 email Validation
	@api.constrains('work_email')
	def email_validating(self):
		for x in self:
			if re.match(r"^[a-zA-Z0-9][\w-]*@[a-zA-Z]+\.[a-zA-Z]{1,3}$",x.work_email)==None:
				raise ValidationError("Please Provide valid Email Address: %s" % x.work_email)



	#Developer2 Mobile Validation
	@api.constrains('mobile_phone')	
	def mobile_validation(self):
		for x in self:
			if len(str(x.mobile_phone))==10:
				res=re.match(r"^0[0-9]{1,10}$",x.mobile_phone)
				if res:
					return True
				else:
	...
	def _onchange_department(self):
		self.line_manager_id = self.department_id.manager_id
...
class Holidays(models.Model):
	...
	def date_from_validation(self):
		...
		for details in holidays:
			if details.year == int(datetime.now().strftime("%Y")):
				for line_ids  in details.line_ids:
					if self.date_from[:10] == str(line_ids.date):
						raise UserError("Your Requested Start Date Holiday. Please Check the Holiday Calender . ")
 					for friday in fridays:
 						if friday[:10] == self.date_from:
 							raise UserError("Your Requested Start Date is Friday. Note: Week off")

		today_date = datetime.now().strftime('%Y-%m-%d')
		if today_date >  self.date_from:
			raise UserError(_("Please Select Valid Date ."))
		self.total_leave_days()
	
	# Developer1 Check the Validate Leave days and Week Off's
	@api.depends('date_from', 'date_to')
	def total_leave_days(self):
		date_from_day = fields.Datetime.from_string(self.date_from)
		date_to_day = fields.Datetime.from_string(self.date_to)
		time_delta = date_to_day-date_from_day
		fridays = self.all_fridays()
		...
		for details in holidays:
			if details.year == int(datetime.now().strftime("%Y")):
				for line_ids  in details.line_ids:
					if self.date_from < line_ids.date and self.date_to > line_ids.date:
						holidays_count = holidays_count + 1
				for friday in fridays:
					if self.date_from < friday and self.date_to > friday:
						fridays_count = fridays_count + 1

		self.number_of_days_temp = math.ceil((time_delta.days + float(time_delta.seconds) / 86400)-holidays_count-fridays_count)
		print math.ceil(time_delta.days + float(time_delta.seconds) / 86400)
		print holidays_count
		print fridays_count
		
	# Developer1 Get Current Year All Friday Dates	
	...
	# Developer1 Date To Field Mandatory Validations
	@api.onchange('date_to')
	@api.constrains('date_to')
	def date_to_validation(self):
		...
		for details in holidays:
			if details.year == int(datetime.now().strftime("%Y")):
				for line_ids  in details.line_ids:
					if self.date_to[:10] == str(line_ids.date):
						raise UserError("Your Requested Last Date Holiday. Please Check the Holiday Calender . ")
					
					for friday in fridays:
 						if friday == self.date_to[:10]:
 							raise UserError("Your Requested Last Date is Friday. Note: Week off")
...
class Applicant(models.Model):
	...
	def _onchange_stage_id_internal(self, stage_id):
		if self.stage_id.name != False and self.last_stage_id.name == True:
	...
	def email_validating(self):
		for x in self:
			if re.match(r"^[a-zA-Z0-9][\w-]*@[a-zA-Z]+\.[a-zA-Z]{1,3}$",x.email_from)==None:
				raise ValidationError("Please Provide valid Email Address: %s" % x.email_from)
	...
	def name_validation_firstname(self):
		for y in self:
			if len(y.partner_name)>=3:
				res=re.match(r"^[^\W0-9_]+([ \-'][^\W0-9_]+)*?$",y.partner_name)
				if res:
					return True
				else:
					raise UserError(_("Your First name must be have Only character"))
