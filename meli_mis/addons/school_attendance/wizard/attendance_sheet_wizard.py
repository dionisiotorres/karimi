...
class StudentAttendanceReport(models.TransientModel):
	...
	def print_report(self, data):
		attendance_line = self.env['daily.attendance.line'].search([('stud_id','=',self.student_id.id)])
		date_domain = []
		if self.from_date:
			date_domain.append(('standard_id.date','>=',self.from_date))
		
		if self.to_date:
			date_domain.append(('standard_id.date','<=',self.to_date))
		if attendance_line:
			att_id = attendance_line.search(date_domain)
			data['form']= att_id.ids
		return self.env['report'].get_action(self, 'school_attendance.student_attendace_report', data=data)
...
class MonthlyAttendanceSheet(models.TransientModel):
	...
	def _compute_standard_id(self):
		for rec in self:
			if self.run_standard_id:
				rec.standard_id =self.run_standard_id.id
