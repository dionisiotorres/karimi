...
class AttendanceSheet(models.Model):
	...
	month_id = fields.Integer('Month', required=False,
							   help="Select Academic Month")
	year_id = fields.Integer('Year')
	...
	def fields_view_get(self, view_id=None,
		...
		st_dates = datetime.strptime(str(start),
...
class StudentleaveRequest(models.Model):
	...
	def default_get(self,vals):
		res = super(StudentleaveRequest, self).default_get(vals)
		if self._context.get('active_id'):
			student_leave = self.search([('student_id','=', self._context.get('active_id'))])
			if len(student_leave) >= self.env['student.student'].browse(self._context.get('active_id')).semester_id.available_leave:
				raise UserError('Your available leaves are over. \n You can not take extra leave')
	...
	def approve_state(self):
		student_leaves = self.env['studentleave.request'].search([
		...
		leave_count = 0
		for val in student_leaves:
			leave_count += val.days
		employee = self.env.ref('school_attendance.student_leave_req_sending_mail')
		self.env['mail.template'].browse(employee.id).send_mail(self.id, force_send=True)
		# rec=self.env['studentleave.request'].search([])
		# for x in rec:
		# 	url = "https://www.fast2sms.com/dev/bulk"
		# 	payload = "sender_id=FSTSMS&message=Dear"+str(x.student_id.name)+" Your Leave Request Was Accepted. " +str(x.days)+" Days Only"+" From "+str(x.start_date)+" To "+str(x.end_date)+". &language=english&route=p&numbers="+str(x.student_id.mobile)
		# 	headers = {
		...
		# if leave_count > 4:
		# 	raise UserError('The number of remaining leaves is not sufficient for this leave type')
	...
	def reject_state(self):
		employee = self.env.ref('school_attendance.student_leave_req_sending_mail1')
		self.env['mail.template'].browse(employee.id).send_mail(self.id, force_send=True)
		rec=self.env['studentleave.request'].search([])
		for x in rec:
			url = "https://www.fast2sms.com/dev/bulk"
			payload = "sender_id=FSTSMS&message=Dear"+str(x.student_id.name)+" Your Leave Request Was Rejected. " +str(x.days)+" Days Only"+" From "+str(x.start_date)+" To "+str(x.end_date)+". &language=english&route=p&numbers="+str(x.student_id.mobile)
			headers = {
						'authorization': "WnE3TqV6AIv5qf8ir5PKnVjXxCzhCFCHasABU58gXRhO9JFqFluWZXvlbsv9",
						'Content-Type': "application/x-www-form-urlencoded",
						'Cache-Control': "no-cache",
	...
	student_id = fields.Many2one('student.student', 'Student', required=True,default=lambda self: self.env['student.student'].search([('user_id', '=', self.env.user.id)]))
...
class AttendanceSheetLine(models.Model):
	...
	def _compute_percentage(self):
		...
		for attendance_sheet_data in self:
			...
			for rec in self:
				data=self.env['daily.attendance'].search_count([('standard_id','=',rec.standard_id.standard_id.id)])
				data1 = self.env['daily.attendance.line'].search_count([('is_present','=',True),('standard_id.standard_id','=',rec.standard_id.standard_id.id),('stud_id','=',rec.student_id.id)])
				percentage = (float(data1) / float(data))* 100
...
class DailyAttendance(models.Model):
	...
	def onchange_check_faculty_value(self):
		if self.user_id:
			...
			self.school_id = self.user_id.school_id
	...
	attendence_by = fields.Many2one('res.users', 'User',default=lambda self: self.env.user)
	...
	def unlink(self):
		...
		return super(DailyAttendance, self).unlink()
			...
			if rec.standard_id:
				...
				for stud in stud_ids:
					...
					student_leave = self.env['studentleave.request'
													   ...
													   ('standard_id', '=',
														rec.standard_id.id),
													   ('course_level','=',rec.standard_id.semester_id.name),
													   ('start_date', '<=',
														rec.date),
													   ('end_date', '>=',
														rec.date)
	...
	def attendance_draft(self):
		...
		for daily_attendance_data in self:
			...
			domain_month =  date.month
			domain = [('standard_id', '=',
					   daily_attendance_data.standard_id.id),
					  ('month_id', '=', domain_month),
					  ]
	...
	def attendance_validate(self):
		...
		for line in self:
			...
			if month_ids:
				...
				domain = [('standard_id','=', line.standard_id.id),('month_id','=', month_data),('year_id','=', year)]
				...
				elif attendance_sheet_id:
					for student_id in line.student_ids:
						domain = [('roll_no', '=', student_id.roll_no),
								  ('standard_id', '=', attendance_sheet_id.id)]
						search_id = sheet_line_obj.search(domain)
						# raise UserError(str(attendance_sheet_id))
						if date.day == 1 and student_id.is_absent:
							val = {'one': False}
		...
		for x in self:
			for y in x.student_ids:
				if y.is_present==False and y.is_absent==False:
					raise ValidationError(_('Please Check Present  and Absent!'))
		for rec in self:
			count = 0
			if rec.student_ids:
				for att in rec.student_ids:
...
class DailyAttendanceLine(models.Model):
	...
	photo = fields.Binary('Images',attachment=True,related='stud_id.photo')
