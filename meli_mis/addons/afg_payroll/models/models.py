...
class payroll_details(models.Model):
	...
	@api.multi
	def SalaryPaymentAction(self):
		test = self.env['account.journal'].search([('code', '=', 'CSH1'),('company_id.name','=',self.campus)], limit=1)
		company_name = self.env['res.company'].search([('name','=',self.campus)], limit=1)
		campus_name = self.env['school.school'].search([('name','=',self.campus)])
		...
		return {
			...
			'context': {
				...
				'default_partner_id':self.env['res.partner'].search([('name','=',self.employee_id.name),('email','=',self.email)]).id,
				'default_journal_id':test.id,
				'default_amount':self.salary_payable,
				# 'currency_id':'INR',
				'default_company':company_name.id,
				'default_communication':self.name,
				'default_school_id':campus_name.id,
				'default_payment_method_id' : 1,
				'default_slip_number':self.number,
			}
		}

	def PaymentStateAction(self):
...
class AccountPaymentInheritsEmpPay(models.Model):
	_inherit = "account.payment"

	@api.constrains('state')
	def StateOnchangeEmpSalaryAction(self):
		obj = self.env['hr.afg.payroll'].search(['|',('number','=',self.slip_number),
												('state','=','done'),
												('name','=',self.communication)])
		print self.state,"@@222222222222222222222@@"
		if self.state == "posted":
			obj.PaymentStateAction()
